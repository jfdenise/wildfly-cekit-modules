   ...
   cat << EOF >> ${CLI_SCRIPT_FILE}
    if (outcome != success) of /socket-binding-group=standard-sockets/socket-binding=http:read-resource
      ...
      exit
        ...
        /socket-binding-group=standard-sockets/socket-binding=http:list-add(name=client-mappings,value={destination-address=$STATEFULSET_HEADLESS_SERVICE_NAME})
    end-if
    if (outcome != success) of /socket-binding-group=standard-sockets/socket-binding=https:read-resource
      ...
      exit
        ...
        /socket-binding-group=standard-sockets/socket-binding=https:list-add(name=client-mappings,value={destination-address=$STATEFULSET_HEADLESS_SERVICE_NAME})
    end-if
EOF
}