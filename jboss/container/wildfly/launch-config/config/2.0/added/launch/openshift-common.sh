...
function exec_cli_scripts() {
  ...
  if [ -s "${script}" ]; then
    ...
    if [ "${SCRIPT_DEBUG}" == "true" ] || [ "${CLI_EXECUTION_OUTPUT}" == "CONSOLE" ]; then
      cat "${CLI_SCRIPT_OUTPUT_FILE}"
    fi
    if [ -n "${CLI_EXECUTION_OUTPUT}" ] && [ "${CLI_EXECUTION_OUTPUT}" != "CONSOLE" ]; then
      cp "${CLI_SCRIPT_OUTPUT_FILE}" "${CLI_EXECUTION_OUTPUT}"
    fi
