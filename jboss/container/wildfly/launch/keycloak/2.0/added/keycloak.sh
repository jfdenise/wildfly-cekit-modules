...
function do_configure_SAML_subsystem() {
 ...
 # We need it to be retrieved prior to iterate the web deployments, needed by CLI
  ...
  do
    if [[ $f != "*.war" ]];then
      keycloak_configure_SAML_secure_deployment "$f" "$realm_signing_key_certificate"
    fi
  done
  popd &> /dev/null
  ...
  for ((deployment_index=1; deployment_index<=count; deployment_index++)); do
    ...
    if [[ "${deployment_runtime_name}" == *.war ]]; then
      ...
      if [ -f "$deployment_file" ]; then
        ...
        pushd /tmp &> /dev/null
        keycloak_configure_SAML_secure_deployment "${deployment_runtime_name}" "${realm_certificate}"
