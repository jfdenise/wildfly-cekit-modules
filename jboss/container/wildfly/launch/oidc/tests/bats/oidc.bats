...
@test "manual configuration" {
   ...
   /subsystem=elytron-oidc-client/realm=demo/:add(realm-public-key="",auth-server-url=https://secure-sso-demo.cloudapps.example.com/auth,ssl-required=external,disable-trust-manager=true,allow-any-hostname=true)
   /subsystem=elytron-oidc-client/secure-deployment="app-profile-jee.war"/:add(realm=demo,resource=app-profile-jee,auth-server-url=https://secure-sso-demo.cloudapps.example.com/auth,allow-any-hostname=true,enable-basic-auth=true,enable-cors=false)
   /subsystem=elytron-oidc-client/secure-deployment="app-profile-jee.war":write-attribute(name=credential.secret,value=KICe8sS2)
EOF
)
    cp $BATS_TEST_DIRNAME/secure-deployments.cli $JBOSS_HOME/standalone/configuration
    run oidc_configure
    echo "CONSOLE: ${output}"
    output=$(<"${CLI_SCRIPT_FILE}")
    normalize_spaces_new_lines
...
@test "sso provider, provider enabled, added non OIDC deployment to be secured with OIDC, generation expected" {
    expected=$(cat << EOF
    if (outcome != success) of /extension=org.wildfly.extension.elytron-oidc-client:read-resource
   /extension=org.wildfly.extension.elytron-oidc-client:add()
   end-if
